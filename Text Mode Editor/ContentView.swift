...
func LoadTiles() -> [[Int]] {
    var tileData: [[Int]] = [[]]
    if let url = Bundle.main.url(forResource: "c64fontmono", withExtension: "txt") {
        if let fileContent = try? String(contentsOf: url) {
            for line in fileContent.components(separatedBy: .newlines) {
                var rowData: [Int] = []
                for char in line {
                    if char.isNumber {
                        rowData.append(char.wholeNumberValue!)
                    }
                }
    ...
    tileData.remove(at: tileData.endIndex-1)
    tileData.remove(at: tileData.startIndex)
    return tileData
}

let tiles: [[Int]] = LoadTiles()
